local function CreateMoistureIndicator(inst)
    local amount = inst.sign * (inst.indicator:value() or 0)
    local absAmount = math.abs(amount);

    local label = inst.entity:AddLabel()

    -- size = 0.5+ln(x)/3+x*x/4000
    -- Which leads to nearly f(0) = 0.5, f(15) = 1, f(100) = 4
    local size = (0.5+(math.log(absAmount+1))/3+(absAmount*absAmount)/4000);
    ...
    local color
    if amount < 0 then
        color = TUNING.MOISTURE_LOSE_COLOR
    else
        color = TUNING.MOISTURE_GAIN_COLOR
    end
...
local function InitDI()
    ...
    inst.isheal = net_bool(inst.GUID, "moiind.sign", "signdirty")
    inst:ListenForEvent("signdirty", SetNumberSign)

    inst.indicator = net_ushortint(inst.GUID, "moiind.number", "numberdirty")
