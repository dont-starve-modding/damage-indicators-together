local function CreateHungerIndicator(inst)
    ...
    local color
    if amount < 0 then
        color = TUNING.HUNGER_LOSE_COLOR
    else
        color = TUNING.HUNGER_GAIN_COLOR
    end
...
local function InitDI()
    ...
    inst.isheal = net_bool(inst.GUID, "hngind.sign", "signdirty")
    inst:ListenForEvent("signdirty", SetNumberSign)

    inst.indicator = net_ushortint(inst.GUID, "hngind.number", "numberdirty")
