local function CreateFueledIndicator(inst)
    local amount = inst.sign * (inst.indicator:value() or 0)
    local absAmount = math.abs(amount);

    local label = inst.entity:AddLabel()

    -- size = 0.5+ln(x)/3+x*x/5000
    -- Which leads to nearly f(0) = 0.5, f(5) = 1, f(100) = 4
    -- This means, lower numbers are half that small than numbers around 5
    -- and 100% is 4 times as big as light of 5
    local size = (0.5+(math.log(absAmount+1))/3+(absAmount*absAmount)/5000);
    ...
    local color
    if amount < 0 then
        color = TUNING.FUELD_LOSE_COLOR
    else
        color = TUNING.FUELD_GAIN_COLOR
    end
...
local function InitDI()
    ...
    inst.isheal = net_bool(inst.GUID, "fulind.sign", "signdirty")
    inst:ListenForEvent("signdirty", SetNumberSign)

    inst.indicator = net_ushortint(inst.GUID, "fulind.number", "numberdirty")
