local function CreateSanityIndicator(inst)
...
local function InitDI()
    ...
    inst.isheal = net_bool(inst.GUID, "sanind.sign", "signdirty")
    inst:ListenForEvent("signdirty", SetNumberSign)

    inst.indicator = net_ushortint(inst.GUID, "sanind.number", "numberdirty")
